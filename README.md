...
**自助退群(`LeaveGroup`)+ 我的群列表(`GetMyGroups`)**:一次业务全流程走查里发现的两个真实缺口——普通成员在没有 `LeaveGroup` 之前只能靠 `RemoveMember`,而这个 RPC 天然要求 operator 有管理权限,导致"自己退出一个群"这件基本操作实际上做不到(唯一出路是求管理员把自己踢出去);同理,客户端要展示"我的群聊"列表时,`GetGroupInfo` 要求已经知道 `group_id`,没有反向查询"我在哪些群里"的接口。修复思路是共享现有不变量而不是重新发明一套:`Group.leaveGroup(userId)` 和 `removeMember` 都走同一个 `removeMemberInternal`,区别只是前者不经过 `requireManager` 权限门槛——但 OWNER 依然不能不转让就退群,这条规则两边一致,不因为是自己走就放松,否则群会变成没有群主的孤儿状态。`GetMyGroups` 走 `t_group_member` 反查 `group_id` 列表再批量加载完整 `Group`(见下面 N+1 修复说明)。已通过真实网关协议(X25519 握手 + AES-GCM 加密,不是直连 im-core gRPC 的走捷径验证方式)重现原始失败场景并确认修复:普通成员自助退群成功、OWNER 未转让时退群仍被拒绝、`GetMyGroups` 返回的群列表跟实际成员关系精确匹配。

`GetMyGroups` 最初的实现是查出 `group_id` 列表后逐个调 `findById` 加载完整 `Group`(代码评审时发现的 N+1:1 次查列表 + N 次 `findById`,而 `findById` 本身又是 2 次查询——群 PO + 全量成员 PO,合计 `1+2N` 次数据库往返,群数量级虽然远小于消息量级,但逐个查仍是不必要的浪费)。已修复:新增 `GroupRepository.findAllByGroupIds`,固定 2 次批量 `IN` 查询(群 PO 一次、全部相关群的成员 PO 一次),不随群数量线性增长——`GroupRepositoryImpl` 内部把 PO→领域对象的映射逻辑从 `findById` 里提出来复用,两个方法共享同一份转换代码。补了单元测试断言 `findById` 不再被逐个调用,并通过直连 im-core gRPC 做了行为回归验证(多个群、部分群有额外成员,`GetMyGroups` 返回的群列表和每个群的 `member_count` 都跟批量查询前完全一致)。
...
**消息发送链路合并群聊解析(性能修复)**:代码评审发现,`MessageWriteService.send()` 里群禁言检查、拉黑检查、收件人解析这三步各自独立判断"这个 `chat_id` 是不是群聊"——每一步都单独调一次 `GroupRepository.findById(chatId)`,而这个方法不只是查群是否存在,还会把全量成员列表也拉出来,群规模大的时候三次重复查询是实打实的浪费。修复思路是把"是不是群聊"这个判断从三个检查类里收敛成 `send()` 里的一次 `groupRepository.findById(chatId)`,结果(`Optional<Group>`)分别传给 `GroupMuteGuard`/`SingleChatBlockGuard`/`RecipientResolver`——三个类不再各自持有 `GroupRepository`,`GroupMuteGuard` 甚至因此变成零依赖的纯函数式判断。`RecipientResolver` 接口新增一个带 `Optional<Group>` 参数的重载(默认实现退回两参数版本),`MessageRecallService`/`ReadCursorService` 等没有预先解析过群的调用方不用跟着改。同时这个改动顺带解决了另一处代码评审发现:`SingleChatBlockGuard` 原来自己重新实现了一遍"判断是不是群聊"的逻辑,跟 `GroupMuteGuard`/`DefaultRecipientResolver` 各写一份、容易失步——现在三处共享同一次解析结果,不再各自维护一份判断逻辑。已补充单元测试(`GroupMuteGuardTest`/`DefaultRecipientResolverTest` 新增,`SingleChatBlockGuardTest` 改造)验证新的调用方式确实跳过了重复查询,并通过直连 im-core gRPC 的方式做了行为回归验证(单聊/群聊正常发送、拉黑仍生效、群禁言仍生效、群主不受全员禁言影响)。

**未读数(`GetUnreadCount`)**:此前客户端只能拿到已读游标(`read_to_message_id`),自己没法算出"还有几条没读"这个最基础的会话列表红点数字。新增 `MessageStore.countAfter(chatId, afterMessageId, excludeSenderId)`(`MongoMessageStore` 里就是一个 `messageId > X` 的 count 查询,跟 `pullHistory` 依赖的是同一条"`message_id` 在同一 `chat_id` 内单调递增"的既有约定,不是新引入的假设),`ReadCursorService.getUnreadCount` 组合已有的 `getReadToMessageId` 和这个新方法。没有做成"一次性拉全部会话未读数"的批量接口——平台本来就不维护"这个用户有哪些单聊会话"的清单(群聊有 `GetMyGroups`,单聊没有对应物,客户端自己跟踪 `chat_id`),批量接口需要先解决这个更大的问题,不是这次要做的事。已通过真实网关协议验证:未读数随发消息增长、`UpdateReadCursor` 后清零、清零后再来一条消息变回 1。

`countAfter` 最初的签名只有 `(chatId, afterMessageId)`,不区分是谁发的消息——代码评审时发现:一个从没调用过 `UpdateReadCursor` 的用户,自己发出去的消息也会被算进自己的未读数(游标为 0,任何比 0 大的 `message_id` 都算未读,包括自己刚发的)。已修复:`countAfter` 加了第三个参数 `excludeSenderId`,`ReadCursorService.getUnreadCount` 传自己的 `userId` 进去,Mongo 查询条件里排掉 `senderId` 等于查询者本人的消息——自己发的消息不该出现在自己的未读列表里。补了单元测试断言 `excludeSenderId` 确实传的是查询者本人,并通过直连 im-core gRPC 做了行为回归验证:只发不读时自己的未读数是 0,对方的未读数正确增长;对方回复后,自己的未读数只算对方发的那条,不把自己之前发的也算进去。
